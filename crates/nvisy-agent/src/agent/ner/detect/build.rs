...
//!
//! Used by the [`detect`] pass to lift LLM-produced candidates
//! into entities. The recognition method is computed per-candidate
//! via the caller-supplied closure: candidates carrying a
//! `hint_id` get [`RecognitionMethod::Annotation`] with the hint's
//! name; fresh discoveries get [`RecognitionMethod::LlmNer`] with
//! the model's provenance.
//!
//! Candidates missing an `entity_type` are dropped (we don't
//! invent a kind); candidates whose `confidence` falls outside
//! `[0, 1]` after clamping are also dropped.
//!
//! [`detect`]: super::NerAgent::detect
//! [`RecognitionMethod::LlmNer`]: nvisy_ontology::entity::RecognitionMethod::LlmNer
//! [`RecognitionMethod::Annotation`]: nvisy_ontology::entity::RecognitionMethod

use nvisy_ontology::entity::{Entity, EntityCategory, RecognitionMethod};
...
/// recognition method per candidate via `recognition_for`. Logs
/// (at debug) the count of candidates dropped for missing kind or
/// bad confidence.
pub(crate) fn build_entities<F>(
    localized: Vec<LocalizedCandidate>,
    mut recognition_for: F,
) -> Vec<Entity<Text>>
where
    F: FnMut(&LocalizedCandidate) -> RecognitionMethod,
{
    ...
    for l in localized {
        ...
        // Trust the LLM's category if it gave one; otherwise derive
        // it from the entity_kind via the ontology mapping. Keeps
        // kind/category internally consistent without a fixed
        // default that would lie about the kind.
        ...
        let recognition = recognition_for(&l);

        let mut b = Entity::builder()
            .with_category(category)
            .with_entity_kind(entity_kind)
            .with_recognition_methods(vec![recognition])
