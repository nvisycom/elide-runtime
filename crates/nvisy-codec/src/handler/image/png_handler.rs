...
///
/// Stores the decoded [`DynamicImage`] directly.  The raw PNG bytes
/// can be produced on demand via [`PngHandler::encode_bytes`].
#[derive(Debug, Clone)]
...
impl PngHandler {
    ...
    /// Encode the current image to PNG bytes.
    pub fn encode_bytes(&self) -> Result<Vec<u8>, Error> {
        let mut buf = std::io::Cursor::new(Vec::new());
        self.image
            .write_to(&mut buf, image::ImageFormat::Png)
            .map_err(|e| Error::new(ErrorKind::Runtime, format!("PNG encode failed: {e}")))?;
        Ok(buf.into_inner())
    }
}
