...
use super::{TextData, TextSpanId, forward_edits, reindex_stream};
...
/// that heterogeneous text handlers can be used interchangeably.
...
/// `CsvSpan`, `JsonPath`). This trait normalises them to [`TextSpanId`]
/// via [`reindex_stream`] and [`forward_edits`], allowing
/// [`BoxedTextHandler`] to present a uniform
/// `TextHandler<TextId = TextSpanId>` interface.
///
...
mod tests {
    ...
    async fn view_spans_returns_text() {
        ...
        assert_eq!(spans[0].id, TextSpanId::new(0));
        assert_eq!(spans[1].id, TextSpanId::new(1));
    }
}
