...
use nvisy_document::pipeline::{RunFilter, RunOutcome, RunStatus};
use uuid::Uuid;
...
async fn dry_run_returns_without_exporting() -> anyhow::Result<()> {
    ...
    assert!(
        output
            .audits
            .iter()
            .all(|a| a.applied_redactions_count() == 0)
    );
    Ok(())
}
...
async fn failed_run_appears_in_listing() -> anyhow::Result<()> {
    ...
    assert_eq!(runs[0].status, nvisy_document::pipeline::RunStatus::Failed);
    Ok(())
}
