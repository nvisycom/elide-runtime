...
//!
//! `PatternRecognizer` is itself modality-generic (impls
//! `Recognizer<M>` for any `M: TextRecognizable`), so this
//! module hosts the pattern-attach helper here alongside the
//! language enricher for text and the dedup pipeline that runs
//! after every modality. NER and LLM attach live in
//! [`super::ner`] and [`super::llm`].
