...
//!
//! Mirrors [`crate::policies`]: contexts are immutable per
...
/// Write a new context version. Fails with [`ErrorKind::Conflict`]
/// when an entry already exists at `(actor_id, context.id, version)`.
///
/// [`ErrorKind::Conflict`]: nvisy_core::ErrorKind::Conflict
pub async fn put(handle: &RegistryHandle, actor_id: Uuid, context: &Context) -> Result<()> {
    ...
    blocking(move || {
            ...
            .map_err(fjall_err)?
        {
            return Err(Error::conflict(
                format!(
                    "context already exists at actor_id={actor_id}, context_id={context_id}, \
...
/// Read the highest-version context for `(actor_id, context_id)`.
///
/// [`ErrorKind::NotFound`]: nvisy_core::ErrorKind::NotFound
pub async fn latest(handle: &RegistryHandle, actor_id: Uuid, context_id: Uuid) -> Result<Context> {
    let contexts = handle.contexts().clone();
    blocking(move || {
...
/// List every `(context_id, version)` pair stored for `actor_id`.
pub async fn list(
    handle: &RegistryHandle,
    actor_id: Uuid,
    ...
    blocking(move || {
        ...
        for guard in contexts.prefix(prefix) {
            ...
            let id = VersionedKey::resource_id_from_bytes(bytes).ok_or_else(|| {
                Error::internal("unexpected context key length", COMPONENT)
            })?;
