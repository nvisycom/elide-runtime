//! Per-deployment `[extraction.*]` configuration that builds the
//! toolkit-side [`ExtractorRegistry`] at engine startup.
//!
//! [`ExtractionConfig::build`] turns each opted-in `[extraction.*]`
//! section into a concrete extractor and inserts it into the
//! [`ExtractorRegistry`]. Per-technique sub-configs
//! ([`OcrExtractorConfig`], [`SttExtractorConfig`]) live in their
//! own files; the build path is centralised here so the two arms
//! of each backend selector enum don't need to unify their
//! concrete types via `dyn` erasure.
//!
//! Per-request extraction plan knobs ([`Extraction`] and the
//! per-modality plan structs) live in [`super::plan`], not here.
//!
//! [`ExtractorRegistry`]: nvisy_toolkit::extraction::ExtractorRegistry
//! [`Extraction`]: super::plan::Extraction
...
///
/// Each field maps to a `[extraction.*]` section in `Nvisy.toml`.
/// A `None` opts the technique out entirely; an opted-in section
/// is built once at engine startup and inserted into the
/// toolkit-side [`ExtractorRegistry`].
///
/// [`ExtractorRegistry`]: nvisy_toolkit::extraction::ExtractorRegistry
...
impl ExtractionConfig {
    ...
    /// inserting the concrete extractor directly so no `dyn`
    /// erasure is required outside the registry's internal
    /// storage.
    ///
    /// # Errors
    ///
