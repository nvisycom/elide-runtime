...
///
/// Cheaply clonable (Arc bump). Both the [`DetectionEngine`][de] and
/// a paired [`RedactionEngine`][re] (via
/// [`RedactionEngine::from_detection`][rfd]) hold clones — the
/// redaction side reads it for the detect→redact handoff path,
/// avoiding a registry round-trip.
///
/// [de]: super::DetectionEngine
...
impl DetectionState {
    ...
    pub(crate) async fn insert(&self, id: Uuid, record: DetectionRecord) {
    ...
    pub(crate) async fn set_started_at(&self, id: Uuid) {
    ...
    pub(crate) async fn fail(&self, id: Uuid, error: impl Into<String>) {
        let mut guard = self.inner.write().await;
        if let Some(record) = guard.get_mut(&id) {
    ...
    /// [`ErrorKind::NotFound`]: nvisy_core::ErrorKind::NotFound
    /// [`ErrorKind::Conflict`]: nvisy_core::ErrorKind::Conflict
    pub(crate) async fn cancel(&self, actor_id: Uuid, id: Uuid) -> Result<(), Error> {
        let mut guard = self.inner.write().await;
    ...
    /// [`ErrorKind::NotFound`]: nvisy_core::ErrorKind::NotFound
    /// [`ErrorKind::Conflict`]: nvisy_core::ErrorKind::Conflict
    pub(crate) async fn delete(&self, actor_id: Uuid, id: Uuid) -> Result<(), Error> {
        let mut guard = self.inner.write().await;
        let Some(record) = guard.get(&id) else {
            return Err(Error::not_found(
                format!("detection {id} not found"),
    ...
    pub(crate) async fn delete_all(&self, actor_id: Uuid) -> Vec<Uuid> {
        let mut guard = self.inner.write().await;
