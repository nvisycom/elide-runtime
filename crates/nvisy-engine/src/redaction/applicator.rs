...
use crate::document::provenance::{
    ...
    RedactionDecision, ResolvedAction,
};
        ...
        match ov {
            ...
            RedactionOverride::Reject { .. } => {
                let entry = record.audit.take().unwrap_or_else(|| AuditEntry {
                    decision: Decision {
                        policy_ref: None,
                        action: ResolvedAction::Suppress { reason: None },
                    },
            ...
            RedactionOverride::Replace {
                ...
                record.audit = Some(AuditEntry {
                    decision: Decision {
                        policy_ref,
                        action: ResolvedAction::Redact { operator: typed },
                    },
    ...
    M: DocumentModality,
{
    ...
    let prebuilt = operator.map(|op| AuditEntry {
        decision: Decision {
            policy_ref: None,
            action: ResolvedAction::Redact { operator: op },
        },
        execution: Execution::Pending,
