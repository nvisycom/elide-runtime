//! Detection-engine construction and basic lifecycle tests.

use std::time::Duration;

use nvisy_engine::core::{EngineConfig, ResourceLimits, RuntimeConfig};
use nvisy_engine::detection::DetectionEngine;

#[tokio::test]
async fn temp_engine_points_to_temp_dir() {
    let (engine, dir) = DetectionEngine::temp().await;
...
async fn open_with_default_config() -> anyhow::Result<()> {
    ...
    let engine = DetectionEngine::open(dir.path(), RuntimeConfig::default()).await?;
    assert_eq!(engine.data_dir(), dir.path());
    Ok(())
}
...
async fn open_with_custom_limits() -> anyhow::Result<()> {
    ...
    let engine = DetectionEngine::open(dir.path(), config).await?;
    assert_eq!(
