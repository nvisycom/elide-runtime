...
use crate::handler::{DocxHandler, ImageHandler, FormatHandler, BinaryLoader};
...
impl Clone for DocxLoader {
    fn clone(&self) -> Self { Self }
}

#[async_trait::async_trait]
impl BinaryLoader for DocxLoader {
    ...
    async fn load(
        ...
        let mut doc = Document::new(DocxHandler)
            .with_text(full_text)
...
impl crate::handler::Handler for DocxLoader {
    fn id(&self) -> &str { DocxHandler.id() }
    fn extensions(&self) -> &[&str] { DocxHandler.extensions() }
    fn content_types(&self) -> &[&str] { DocxHandler.content_types() }
}
