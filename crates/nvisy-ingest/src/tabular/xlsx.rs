...
use crate::handler::{XlsxHandler, PlaintextHandler, FormatHandler, SpreadsheetLoader};
...
impl Clone for XlsxLoader {
    fn clone(&self) -> Self { Self }
}

#[async_trait::async_trait]
impl SpreadsheetLoader for XlsxLoader {
    ...
    async fn load(
        ...
        let cursor = Cursor::new(content.to_bytes().to_vec());
        ...
        for sheet_name in &sheet_names {
            ...
            let mut tabular_doc = Document::new(XlsxHandler)
                .with_tabular(columns, rows)
                .with_sheet_name(sheet_name);
            tabular_doc.source.set_parent_id(Some(content.content_source.as_uuid()));
            documents.push(tabular_doc.into_format());
        }

        // Create a flattened document for regex/dictionary scanning
        if !all_text_parts.is_empty() {
            let mut doc = Document::new(PlaintextHandler)
                .with_text(all_text_parts.join("\n"));
...
impl crate::handler::Handler for XlsxLoader {
    fn id(&self) -> &str { XlsxHandler.id() }
    fn extensions(&self) -> &[&str] { XlsxHandler.extensions() }
    fn content_types(&self) -> &[&str] { XlsxHandler.content_types() }
}
