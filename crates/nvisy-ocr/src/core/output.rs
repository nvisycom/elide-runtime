//! OCR output types.
//!
//! The structural types ([`Word`], [`Line`], [`Block`], [`Page`])
//! are re-exported from [`artifacts`].
//!
...
/// [`Page`] → [`Block`] → [`Line`] → [`Word`]. Caller-side
/// provenance (e.g. mapping the output back to the source image)
/// is tracked outside the backend, alongside the call site that
/// issued the request.
