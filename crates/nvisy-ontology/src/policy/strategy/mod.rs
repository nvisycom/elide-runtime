...
//! ([`TextStrategy`], [`ImageStrategy`], [`AudioStrategy`]) pairing a
//! redaction method with its parameters. The modality's
//! [`Modality::Strategy`] associated type points at the appropriate
//! one; consumers parameterise their own types over `M::Strategy`
//! (e.g. `AuditEntry<M>`, `StrategyPolicy<M>`).
//!
