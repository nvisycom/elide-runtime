...
/// consume this through `GET /files` (list) and
/// `GET /files/{id}` (one).
...
pub struct FileMetadata {
    ...
    ///
    /// The `(actor_id, id)` pair keys both the metadata blob and
    /// the raw bytes.
    pub id: Uuid,
    /// Original filename, when the upload carried one.
    ///
    /// E.g. via `Content-Disposition: attachment; filename="scan.pdf"`.
    ...
    /// leading dot. Required at upload time. The engine derives
