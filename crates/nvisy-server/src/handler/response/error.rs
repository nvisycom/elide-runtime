...
pub struct ErrorResponse<'a> {
    /// Machine-readable error name in `SCREAMING_SNAKE_CASE`
    /// (e.g. `"NOT_FOUND"`, `"BAD_REQUEST"`). Stable across releases
    /// for programmatic dispatch — the doc comment uses the same
    /// case the wire emits.
    pub name: Cow<'a, str>,
